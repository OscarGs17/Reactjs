...
import '../assets/styles/personajes.css'; // Asegúrate de que ahí agregues el nuevo estilo
...
const Guardianes = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:3000/guardianes")
    ...
    const manejarSeleccion = (alias) => {
        fetch(`http://localhost:3000/guardianes/${alias}`)
            .then(res => res.json())
            .then(datos => {
    ...
    const eliminarPersonaje = (alias) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`http://localhost:3000/guardianes/eliminar/${alias}`, {
                    method: 'DELETE',
                    headers: {
            ...
            <div className='container'>
                <div className="d-flex justify-content-between align-items-center mb-4 text-white">
                    <div className="d-flex align-items-center">
                        <i className="bi bi-rocket-takeoff-fill text-info fs-1 me-3"></i>
                        <h1 className='fw-bold'>Guardianes de la galaxia</h1>
                    </div>
                    {
